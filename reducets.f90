! Compile (mistral)
! ifort reduceps.f90 -o reduceps `/sw/rhel6-x64/netcdf/netcdf_fortran-4.4.2-intel14/bin/nf-config --fflags --flibs`
! Compile (blizzard):
! xlf95_r -o reducets reducets.f90 `/sw/aix61/netcdf-4.2.1.1/bin/nf-config --fflags --flibs` 
! Compile (thunder):
! ifort -o reducets reducets.f90 -L/sw/sles10-x64/netcdf-3.6.3-intel/lib -lnetcdf -I/sw/sles10-x64/netcdf-3.6.3-intel/include
!new path: /groups/ocean/software/netcdf4/gcc/4.4.1.1/
...
! Assume number of time steps nt smaller than 10000 and number of variables nv smaller than 35 ! otherwise need to be changes
        ...
        integer, parameter :: nv=100,nma=8,nmi=3,nsu=28  ! Christopher: nma+1 (zc), nmi+1 (zb)
        integer :: nt
...
!----------------------------------
	     ...
	     ! Christopher: correct bug
             varout(k,:) = -999.
             do l=1,ntr
              !varout(k,l) = -999.
              !if ((varout(k,l).eq.0).or.((var(k,l).gt.-999).and.(var(k,l).lt.varout(k,l)))) varout(k,l)=var(k,l)
              if ((varout(k,l).eq.-999.).or.(var(k,l).lt.varout(k,l))) varout(k,l)=var(k,l)
             end do
            endif
           end if
          end do
...
!* Define and store variable
      do k=1,nvar
       !Christopher: change unit
       !lmax (kg/kg --> g/kg)
       if (trim(namout(k)).eq.'lmax') then
         uname(k)='g/kg'
	 varout(k,1:ntr) = varout(k,1:ntr)*1000.
       endif
       !b_flux (m^2/s^3)
       if (trim(namout(k)).eq.'b_flux') then
         uname(k)='m^2/s^3'
       endif
       !Christopher: end
